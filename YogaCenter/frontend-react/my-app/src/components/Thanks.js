...
export default function Thanks() {
  ...
  useEffect(() => {
    ...
    await axios.get(customerAPI).then(r => {setCustomer(r.data);
    axios.get(courseAPI).then(r => {setCourse(r.data)
  ...
  if(course !== undefined && customer !== undefined && user.timeout === HmacSHA512(params.get("vnp_TxnRef"),secretKey).toString()){
    console.log(1);
    removeCookie();
    createInvoice();
  }
  ...
  async  function createInvoice(){
        if(params.get("vnp_TransactionStatus") === '00' ){
          await axios.post(postInvoiceAPI,{
            id: invoiceId,
